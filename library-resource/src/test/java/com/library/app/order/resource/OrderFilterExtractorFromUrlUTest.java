...
public class OrderFilterExtractorFromUrlUTest {
	...
	public void withPaginationAndStartDateAndEndDateAndCustomerIdAndStatusAndSortAscending() {
		setUpUriInfo("2", "5", "2017-02-01T10:02:01Z", "2017-02-01T10:02:02Z", "10", OrderStatus.CANCELLED.name(),
				"createdAt");
		final OrderFilterExtractorFromUrl extractor = new OrderFilterExtractorFromUrl(uriInfo);
		final OrderFilter orderFilter = extractor.getFilter();
		...
		assertActualPaginationDataWithExpected(orderFilter.getPaginationData(), new PaginationData(10, 5, "createdAt",
				OrderMode.ASCENDING));
		assertFieldsOnFilter(orderFilter, DateUtils.getAsDateTime("2017-02-01T10:02:01Z"),
				DateUtils.getAsDateTime("2017-02-01T10:02:02Z"), 10L, OrderStatus.CANCELLED);
	}

	private void setUpUriInfo(final String page, final String perPage, final String startDate,
			final String endDate, final String customerId, final String status, final String sort) {
