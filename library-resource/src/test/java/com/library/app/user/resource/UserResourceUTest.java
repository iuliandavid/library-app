...
public class UserResourceUTest {
	...
	public void addValidCustomer() {
		...
		when(userServices.add(userEq(johnDoe())))
				.thenReturn(userWithIdAndCreatedAt(johnDoe(), 1L));

		final Response response = userResource
				.add(readJsonFile(getPathFileRequest(PATH_RESOURCE, "customerJohnDoe.json")));
	...
	public void addUserWithNullName() {
		...
		when(userServices.add((User) any()))
				.thenThrow(new FieldNotValidException("name", "User Name cannot be null"));

		final Response response = userResource
				.add(readJsonFile(getPathFileRequest(PATH_RESOURCE, "userWithNullName.json")));
		assertThat(response.getStatus(), is(equalTo(HttpCode.VALIDATION_ERROR.getCode())));
	...
	public void updateUserWithNullName() {
		...
		doThrow(new FieldNotValidException("name", "User Name cannot be null")).when(userServices).update(
	...
	public void findByFilterNoFilter() {
		...
		when(userServices.findByFilter((UserFilter) any())).thenReturn(
