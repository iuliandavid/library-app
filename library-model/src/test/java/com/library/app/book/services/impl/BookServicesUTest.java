...
public class BookServicesUTest {
	...
	public void addValidBook() throws Exception {
		when(categoryServices.findById(null)).thenReturn(cleanCode().getCategory());
		when(authorServices.findById(null)).thenReturn(robertMartin());
	...
	public void findBookByFilter() {
		...
		when(bookRepository.findByFilter((BookFilter) any())).thenReturn(books);

		final PaginatedData<Book> booksReturned = bookServices.findByFilter(new BookFilter());
		assertThat(booksReturned.getNumberOfRows(), is(equalTo(books.getNumberOfRows())));
		assertThat(booksReturned.getRow(0).getTitle(), is(equalTo(books.getRow(0).getTitle())));
	}
