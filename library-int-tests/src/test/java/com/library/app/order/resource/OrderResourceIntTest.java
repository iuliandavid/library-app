...
public class OrderResourceIntTest {
	...
	public void addValidOrderAndFindIt() {
		final Long orderToBeFound = addOrderAndGetId(normalizeDependenciesWithRest(orderReserved()));
		findOrderAndAssertResponseWithOrder(orderToBeFound, orderReserved());
	}
	...
	private void assertResponseContainsTheOrdersStatus(final Response response, final int expectedTotalRecords,
		...
		final JsonArray ordersList = IntegrationTestUtils.assertJsonHasTheNumberOfElementsAndReturnTheEntries(response,
	...
	private Long addOrderAndGetId(final Order order) {
		return IntegrationTestUtils.addElementWithContentAndGetId(resourceClient.user(johnDoe()), PATH_RESOURCE,
	...
	private Order normalizeDependenciesWithRest(final Order order) {
		order.getItems().forEach((item) -> item.getBook().setId(loadBookIdFromRest(item.getBook())));
		return order;
	}
	...
	private Long loadBookIdFromRest(final Book book) {
		final Response response = resourceClient
				.resourcePath(
						"DB/" + ResourceDefinitions.BOOK.getResourceName() + "/" + book.getTitle())
				.get();
		assertThat(response.getStatus(), is(equalTo(HttpCode.OK.getCode())));
		final String bookResult = response.readEntity(String.class);
		assertThat(bookResult, is(notNullValue()));
		final JsonObject bookAsJson = JsonReader.readAsJsonObject(bookResult);
		assertThat(JsonReader.getLongOrNull(bookAsJson, "id"), is(notNullValue()));
		return JsonTestUtils.getIdFromJson(bookResult);
	}

	private void findOrderAndAssertResponseWithOrder(final Long orderToBeFound, final Order expectedOrder) {
		final String bodyResponse = IntegrationTestUtils.findById(resourceClient, PATH_RESOURCE, orderToBeFound);
		...
		assertThat(orderJson.getAsJsonObject("customer").get("name").getAsString(),
